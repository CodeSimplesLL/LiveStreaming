...
/*添加pps和sps到头部*/
void add_264_pps_sps_packet(unsigned char *pps, unsigned char *sps,
                            int pps_len, int sps_len) {
    /*body内容的包裹需要16字节*/
    int body_size = 13 + sps_len + 3 + pps_len;
    RTMPPacket *packet = (RTMPPacket *) malloc(sizeof(RTMPPacket));
    /*10个成员，至少要设置好7个*/
    /*初始化packet内存区，最大头部加上 body_size */
    ...
    /*取sps_len值，为长度信息*/
    /*取后8-16位*/
    packet_body[i++] = (sps_len >> 8) & 0xff;
    /*取后8位*/
    packet_body[i++] = sps_len & 0xff;
    /*写入sps内容*/
    memcpy(&packet_body[i], sps, sps_len);
    i += sps_len;

    /*代表pps*/
    packet_body[i++] = 0x01;
    packet_body[i++] = (pps_len >> 8) & 0xff;
    packet_body[i++] = (pps_len) & 0xff;
    memcpy(&packet_body[i], pps, pps_len);
    i += pps_len;

    /*开始设置packet其它参数*/
    packet->m_packetType = RTMP_PACKET_TYPE_VIDEO;
    packet->m_nChannel = 0x04;
//    packet->m_nTimeStamp = RTMP_GetTime() - start_time; pps和sps不需要设置时间戳
    packet->m_nTimeStamp = 0;
    packet->m_hasAbsTimestamp = 0;
...
void add_264_IDR_packet(unsigned char *buf, int len) {
    /*去掉帧界定符,两种类型的界定符 *00 00 00 01*/
    if (buf[2] == 0x00) { //
        buf += 4;
        len -= 4;
...
//	packet->m_nTimeStamp = -1;
    /*时间戳为当前时间减掉开始时间，表示推了多久了，属于相对时间*/
    packet->m_nTimeStamp = RTMP_GetTime() - start_time;
    add_rtmp_packet(packet);
}

void add_packets(int i_nal, const x264_nal_t *p_nal){int sps_len, pps_len;
    unsigned char sps[100];
    unsigned char pps[100];
    memset(sps, 0, 100);
    memset(pps, 0, 100);
    for (int i = 0; i < i_nal; i++) {
        if (p_nal[i].i_type == NAL_SPS) { //sps
            sps_len = p_nal[i].i_payload - 4;
            /*读取sps*/
            memcpy(sps, p_nal[i].p_payload + 4, sps_len);
        } else if (p_nal[i].i_type == NAL_PPS) { //pps
            pps_len = p_nal[i].i_payload - 4;
            /*读取pps*/
            memcpy(pps, p_nal[i].p_payload + 4, pps_len);
            add_264_pps_sps_packet(pps, sps, pps_len, sps_len);
        } else {
            /*发送普通帧(剩下全部)*/
            add_264_IDR_packet(p_nal[i].p_payload, p_nal[i].i_payload);
        }
    }
}
...
JNIEXPORT void JNICALL Java_com_lythonliu_live_jni_PusherNative_setVideoOptions(
    ...
    if (x264Handler) {
        ...
        if (mFps != fps || mBitrate != bitrate || mHeight != height
            || mWidth != width) {
            //属性不同
            x264_encoder_close(x264Handler);
            x264Handler = 0;
            free(pic_in);
            free(pic_out);
        } else {
...
//    码率控制相关设置
    ...
    x264Handler = x264_encoder_open(&param);
    if (!x264Handler) {
        LOGI("视频编码器打开失败");
...
JNIEXPORT void JNICALL Java_com_lythonliu_live_jni_PusherNative_setAudioOptions(
...
JNIEXPORT void JNICALL Java_com_lythonliu_live_jni_PusherNative_startPusher(
...
JNIEXPORT void JNICALL Java_com_lythonliu_live_jni_PusherNative_fireAudio(
...
JNIEXPORT jint JNICALL Java_com_lythonliu_live_jni_PusherNative_getInputSamples() {
    return nInputSamples;
}

JNIEXPORT void JNICALL Java_com_lythonliu_live_jni_PusherNative_fireVideo(
        JNIEnv *env, jobject thiz, jbyteArray buffer) {
    if (!publishing || !readyRtmp || !x264Handler || !rtmp
        || !RTMP_IsConnected(rtmp)) {
        return;
    }
    ...
    for (int i = 0; i < u_v_len; i++) {
        /*stored v*/
        *(v_plane + i) = *(nv21_buffer + y_len + i * 2);
        /*stored u*/
        *(u_plane + i) = *(nv21_buffer + y_len + i * 2 + 1);
    }
    int i_nal = -1;
    ...
    *  allocates its own x264_picture_t as opposed to using x264_picture_alloc. */
    ...
    LOGI("开始编码");
    if (x264_encoder_encode(x264Handler, &p_nal, &i_nal, pic_in, pic_out) < 0) {
        LOGI("编码失败");
        return;
    }
    LOGI("编码完成");
    pic_in->i_pts++;
    add_packets(i_nal, p_nal);
    env->ReleaseByteArrayElements(buffer, nv21_buffer, 0);
}

JNIEXPORT void JNICALL Java_com_lythonliu_live_jni_PusherNative_stopPusher(
...
JNIEXPORT void JNICALL Java_com_lythonliu_live_jni_PusherNative_release(
        JNIEnv *env, jobject thiz) {
    Java_com_lythonliu_live_jni_PusherNative_stopPusher(env, thiz);
    if (audioEncHandle) {
